...
class GolemMapper:
    ...
    def cache(self):
        try:
            cache = json.load("/golem/work/cache.json")
            new_idx = 0
            for key in cache:
                new_idx +=1
            cache[new_idx] = self.current
            json.dump("/golem/work/cache.json", cache)
        except Exception as e:
            print(e)
            # init.
            json.dump("/golem/work/cache.json", {0: self.current})

    def load_cache(self, idx):
        try:
            cache = json.load("/golem/work/cache.json")
            self.current = cache[idx]
        except Exception as e:
            pass
